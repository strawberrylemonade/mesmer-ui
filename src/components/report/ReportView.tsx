...
const ReportView: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<IStepReport | Event>()
      ...
      <div className="bg-white shadow overflow-scroll sm:rounded-lg">
        { report ? <ul> 
          { report.steps.map(step => {
            ...
            return <ReportStepItem onClick={(item) => setSelectedItem(item)}  key={step.name} step={step} events={events}></ReportStepItem>
          }) } 
        </ul>  : <div className="mt-12"><Spinner></Spinner></div> }
      </div>
      <Card className={!selectedItem ? 'pointer-events-none opacity-50' : ''}>
        { selectedItem ? <h1>
          sadsad
        </h1> :
